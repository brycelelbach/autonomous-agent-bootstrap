...
Source branches do not track `bootstrap.bash`. Make behavioral changes in the ordered modules under `src/`; `python3 tools/compile_bootstrap.py` writes an ignored local artifact when one is needed. The test harness compiles the artifact before exercising it, and the publish workflow writes the curlable copy to a generated branch.
