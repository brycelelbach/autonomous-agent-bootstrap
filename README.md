...
13. **Agent plugins** listed in [`agent_plugins.txt`](./agent_plugins.txt), installed into Claude Code and Codex, plus the Pi packages listed in [`pi_plugins.txt`](./pi_plugins.txt). AAB-owned Pi repositories follow their default branches, while third-party packages remain version-pinned. Pi uses upstream `nicobailon/pi-subagents`; Autocuda contributes its Pi-native worker through its own package manifest.
