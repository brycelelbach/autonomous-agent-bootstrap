...
2. **[Codex CLI](https://developers.openai.com/codex/cli)** — installed via OpenAI's standalone installer, then configured for unattended use:
   ...
   - Model selected via `AAB_CODEX_FIRST_PARTY_MODEL` (defaults to `gpt-5.5`) for OpenAI or `AAB_CODEX_THIRD_PARTY_MODEL` (defaults to `openai/openai/gpt-5.5`) when `AAB_CODEX_INFERENCE_PROVIDER=third-party`, reasoning effort via `AAB_CODEX_EFFORT` (defaults to `xhigh`), service tier via `AAB_CODEX_SERVICE_TIER` (defaults to `priority`)
