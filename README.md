...
2. **[Codex CLI](https://developers.openai.com/codex/cli)** — installed via OpenAI's standalone installer, then configured for unattended use:
   ...
   - `shell_environment_policy.inherit = "all"` and `ignore_default_excludes = true` so spawned commands can see credential env vars such as `GH_TOKEN`, `OPENAI_API_KEY`, and `AAB_CODEX_THIRD_PARTY_AUTH_TOKEN`
   - Model selected via `AAB_CODEX_FIRST_PARTY_MODEL` (defaults to `gpt-5.5`) for OpenAI or `AAB_CODEX_THIRD_PARTY_MODEL` (defaults to `openai/openai/gpt-5.5`) when `AAB_CODEX_INFERENCE_PROVIDER=third-party`, reasoning effort via `AAB_CODEX_EFFORT` (defaults to `xhigh`), service tier via `AAB_CODEX_SERVICE_TIER` (defaults to `priority`), and subagent thread cap via `AAB_CODEX_AGENT_MAX_THREADS` (defaults to `16`)
   ...
   - `AAB_CODEX_THIRD_PARTY_AUTH_TOKEN` exported when `AAB_CODEX_INFERENCE_PROVIDER=third-party`, with `~/.codex/config.toml` pointed at an OpenAI-compatible Responses endpoint
...
To run Codex through an OpenAI-compatible third-party endpoint instead of OpenAI, add these Codex-specific variables to any recipe:

```bash
export AAB_CODEX_INFERENCE_PROVIDER="third-party"
export AAB_CODEX_THIRD_PARTY_MODEL="openai/openai/gpt-5.5"
export AAB_CODEX_THIRD_PARTY_BASE_URL="https://inference-api.nvidia.com/v1"
export AAB_CODEX_THIRD_PARTY_AUTH_TOKEN="..."
```
...
| `AAB_CODEX_INFERENCE_PROVIDER` | Codex inference provider: `openai` (default, also accepts `first-party`) or `third-party`. When set to `third-party`, `~/.codex/config.toml` uses a custom `model_providers` entry pointed at an OpenAI-compatible Responses API. |
| `AAB_CODEX_FIRST_PARTY_MODEL` | Codex first-party model name. Baked into `~/.codex/config.toml`'s `model` field. Defaults to `gpt-5.5`. |
| `AAB_CODEX_THIRD_PARTY_MODEL` | Codex model ID when `AAB_CODEX_INFERENCE_PROVIDER=third-party`. Defaults to `openai/openai/gpt-5.5`. |
| `AAB_CODEX_THIRD_PARTY_BASE_URL` | OpenAI-compatible base URL for Codex when `AAB_CODEX_INFERENCE_PROVIDER=third-party`. Defaults to `https://inference-api.nvidia.com/v1`. |
| `AAB_CODEX_THIRD_PARTY_AUTH_TOKEN` | Third-party auth token for Codex. Exported from the managed `~/.bashrc` block, mirrored into `/etc/environment`, and referenced by `~/.codex/config.toml` via `env_key = "AAB_CODEX_THIRD_PARTY_AUTH_TOKEN"`. |
...
| `AAB_CODEX_AGENT_MAX_THREADS` | Maximum number of concurrently open Codex subagent threads. Baked into `~/.codex/config.toml`'s `[agents].max_threads` field. Defaults to `16`; invalid values fall back to `16`. |
...
| `~/.codex/config.toml` | Overwritten with unattended Codex defaults while preserving existing Codex plugin marketplace/plugin tables: `approval_policy = "never"`, `sandbox_mode = "danger-full-access"`, `service_tier = "priority"` by default, `[agents].max_threads = 16` by default, `web_search = "live"`, `check_for_update_on_startup = false`, credential-preserving shell env inheritance, and trusted entries for `$HOME` plus the bootstrap launch directory. Existing file backed up to `config.toml.bak.<timestamp>` before the rewrite. |
| `~/.codex/auth.json` | Written by `codex login --with-api-key` when `AAB_CODEX_FIRST_PARTY_API_KEY` is set and `AAB_CODEX_INFERENCE_PROVIDER=openai`, selecting Codex API-key auth for first launch. Third-party Codex uses `AAB_CODEX_THIRD_PARTY_AUTH_TOKEN` from the environment instead. |
...
- The `~/.bashrc` managed block is replaced, not appended — so re-running **without** `AAB_CLAUDE_CODE_FIRST_PARTY_API_KEY` / `AAB_CODEX_FIRST_PARTY_API_KEY` / `AAB_CODEX_THIRD_PARTY_AUTH_TOKEN` / `AAB_BREV_API_KEY` / `AAB_BREV_ORG_ID` / `AAB_GH_TOKEN` set drops a previously-written export. If you want an export to persist across re-runs, keep the env var set when you re-run.
...
- The `/etc/environment` managed block is replaced in place on re-run, mirroring the same resolved-at-bootstrap-time provider / model / token state that goes into `~/.bashrc`, including `AAB_CODEX_THIRD_PARTY_AUTH_TOKEN` when `AAB_CODEX_INFERENCE_PROVIDER=third-party`. The runtime `claude_code_switch_inference_provider` shell function only updates `~/.bashrc` (interactive sessions); to make a switch visible to non-interactive shells (ssh remote command, systemd `EnvironmentFile=`), re-run the bootstrap with the new provider.
...
**`--smoke` spends real inference.** It runs `claude --dangerously-skip-permissions -p` and `codex exec` with a short prompt. When `AAB_CLAUDE_CODE_FIRST_PARTY_API_KEY`, `AAB_CLAUDE_CODE_THIRD_PARTY_*`, `AAB_CODEX_FIRST_PARTY_API_KEY`, or `AAB_CODEX_THIRD_PARTY_AUTH_TOKEN` are present in the local environment, the smoke test maps them to the runtime variables those CLIs read for that invocation.
