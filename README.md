...
2. **[Codex CLI](https://developers.openai.com/codex/cli)** — installed via OpenAI's standalone installer, then configured for unattended use:
   ...
   - Model selected via `AAB_CODEX_FIRST_PARTY_MODEL` (defaults to `gpt-5.5`) for OpenAI or `AAB_CODEX_THIRD_PARTY_MODEL` (defaults to `openai/openai/gpt-5.5`) when `AAB_CODEX_INFERENCE_PROVIDER=nvidia`, reasoning effort via `AAB_CODEX_EFFORT` (defaults to `xhigh`), service tier via `AAB_CODEX_SERVICE_TIER` (defaults to `priority`)
   ...
   - `AAB_CODEX_THIRD_PARTY_AUTH_TOKEN` exported when `AAB_CODEX_INFERENCE_PROVIDER=nvidia`, with `~/.codex/config.toml` pointed at the NVIDIA OpenAI-compatible Responses endpoint
...
| `AAB_CODEX_THIRD_PARTY_MODEL` | Codex NVIDIA model ID when `AAB_CODEX_INFERENCE_PROVIDER=nvidia`. Defaults to `openai/openai/gpt-5.5`. The NVIDIA catalog also exposes `azure/openai/gpt-5.5` as an alternate GPT-5.5 route. |
| `AAB_CODEX_THIRD_PARTY_BASE_URL` | NVIDIA OpenAI-compatible base URL for Codex when `AAB_CODEX_INFERENCE_PROVIDER=nvidia`. Defaults to `https://inference-api.nvidia.com/v1`. |
| `AAB_CODEX_THIRD_PARTY_AUTH_TOKEN` | Third-party auth token for Codex. For `AAB_CODEX_INFERENCE_PROVIDER=nvidia`, use the NVIDIA API key value. Exported from the managed `~/.bashrc` block, mirrored into `/etc/environment`, and referenced by `~/.codex/config.toml` via `env_key = "AAB_CODEX_THIRD_PARTY_AUTH_TOKEN"`. |
...
| `~/.codex/auth.json` | Written by `codex login --with-api-key` when `AAB_CODEX_FIRST_PARTY_API_KEY` is set and `AAB_CODEX_INFERENCE_PROVIDER=openai`, selecting Codex API-key auth for first launch. NVIDIA-backed Codex uses `AAB_CODEX_THIRD_PARTY_AUTH_TOKEN` from the environment instead. |
...
- The `/etc/environment` managed block is replaced in place on re-run, mirroring the same resolved-at-bootstrap-time provider / model / token state that goes into `~/.bashrc`, including `AAB_CODEX_THIRD_PARTY_AUTH_TOKEN` when `AAB_CODEX_INFERENCE_PROVIDER=nvidia`. The runtime `claude_code_switch_inference_provider` shell function only updates `~/.bashrc` (interactive sessions); to make a switch visible to non-interactive shells (ssh remote command, systemd `EnvironmentFile=`), re-run the bootstrap with the new provider.
